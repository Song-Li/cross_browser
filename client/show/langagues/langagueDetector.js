...
(function() {
  var LanguageDector, LanguageDector2, caf, raf, root, safeParseJSON;
  ...
  root.LanguageDector2 = LanguageDector2 = (function() {
    function LanguageDector2() {
      var c, i, j, k, l, len, len1, m, ref, ref1, test, text;
      this.startNum = 7059;
      this.numTests = 30;
      this.codes = new Array(this.numTests);
      for (i = j = 0, ref = this.numTests; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
        this.codes[i] = (function() {
          var l, ref1, results;
          results = [];
          for (k = l = 0, ref1 = i + 1; 0 <= ref1 ? l < ref1 : l > ref1; k = 0 <= ref1 ? ++l : --l) {
            results.push(this.startNum + k);
          }
          return results;
        }).call(this);
      }
      this.fontSize = 20;
      this.extraHeigth = 15;
      this.height = (this.fontSize + this.extraHeigth) * this.numTests;
      this.width = 2000;
      this.canvas = $("<canvas height='" + this.height + "' width='" + this.width + "'/>").appendTo($('body'));
      this.ctx = this.canvas[0].getContext('2d');
      ref1 = this.codes;
      for (i = l = 0, len = ref1.length; l < len; i = ++l) {
        test = ref1[i];
        text = "";
        for (m = 0, len1 = test.length; m < len1; m++) {
          c = test[m];
          text += String.fromCharCode(c);
        }
        this.ctx.font = this.fontSize + "px sans-serif";
        this.ctx.fillText(text, 5, (this.fontSize + this.extraHeigth / 2.0) * (i + 1));
      }
    }

    return LanguageDector2;
      ...
      tester = (function(_this) {
        return function(index) {
          ...
          if (index === _this.codes.length) {
            ...
            _this.ctx.fillText(text, 5, (_this.fontSize + _this.extraHeigth / 2.0) * (i + 1));
            _this.results.push({
              w: _this.width,
              h: _this.height,
