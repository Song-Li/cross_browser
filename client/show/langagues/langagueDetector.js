...
(function() {
  ...
  root.LanguageDector = LanguageDector = (function() {
    function LanguageDector() {
      ...
      this.results = [];
      this.boxTester = Module().cwrap('boxTester', 'number', ['string', 'number', 'number']);
    }

    LanguageDector.prototype.testIfBoxes = function(pixels, rows, cols) {
      var B, G, L, R, binaryImage, i, j, k, len, pix, raw, ref;
      ...
      raw = "";
      for (k = 0, len = binaryImage.length; k < len; k++) {
        pix = binaryImage[k];
        raw += String.fromCharCode(pix);
      }
      return this.boxTester(raw, rows, cols);
    };
