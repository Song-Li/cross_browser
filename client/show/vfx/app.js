        ...
        if (vsErr) {
            ...
            loadTextResource(root + 'shader.fs.glsl', function (fsErr, fsText) {
                if (fsErr) {
                    ...
                    loadJSONResource(root + 'Susan.json', function (modelErr, modelObj) {
                        if (modelErr) {
                            ...
                            loadImage(root + 'color.png', function (imgErr, img) {
                                if (imgErr) {
                                    ...
                                    RunDemo(vsText, fsText, img, modelObj, 'model_tiles');
                                }
                            });
                            loadImage(root + 'wood.png', function (imgErr, img) {
                                if (imgErr) {
                                    alert('Fatal error getting Susan texture (see console)');
                                    console.error(imgErr);
                                } else {
                                    RunDemo(vsText, fsText, img, modelObj, 'model_wood');
                                }
                            });
                        }
                    });
                        ...
                        if (modelErr) {
                            ...
                            loadImage(root + 'color.png', function (imgErr, img) {
                                if (imgErr) {
                                    ...
                                    RunDemo(vsText, fsText, img, modelObj, 'simple_color');
                                }
                            });
                            loadImage(root + 'wood.png', function (imgErr, img) {
                                if (imgErr) {
                                    ...
                                    RunDemo(vsText, fsText, img, modelObj, 'simple_wood');
                                }
                            });
                        }
                    });
                    loadJSONResource(root + 'cube.json', function (modelErr, modelObj) {
                        if (modelErr) {
                            alert('Fatal error getting Susan model (see console)');
                            console.error(fsErr);
                        } else {
                            loadImage(root + 'color.png', function (imgErr, img) {
                                if (imgErr) {
                                    ...
                                    RunDemo(vsText, fsText, img, modelObj, 'cube_no_texture');
                                }
                            });
                        }
                    });
...
var RunDemo = function (vertexShaderText, fragmentShaderText, SusanImage, SusanModel, canvasName) {
  ...
  gl.clearColor(0.0, 0.0, 0.0, 0.0);
    ...
    if(canvasName == 'model_tiles' || canvasName == 'model_wood') mat4.lookAt(viewMatrix, [0, 0, -5], [0, 0, 0], [0, 1, 0]);
    else if(canvasName == 'simple_wood' || canvasName == 'simple_color') mat4.lookAt(viewMatrix, [0, 0, -110], [0, 0, 0], [0, 1, 0]);
    else
        mat4.lookAt(viewMatrix, [0, 0, -150], [0, 0, 0], [0, 1, 0]);
  ...
  var offsetLocation = gl.getUniformLocation(program, "offset[0]");
  gl.uniform1fv(offsetLocation, offset);
  var weightLocation = gl.getUniformLocation(program, "weight[0]");
  gl.uniform1fv(weightLocation, weight);
  /*var kernelLocation = gl.getUniformLocation(program, "kernel[0]");
  var kernelWeightLocation = gl.getUniformLocation(program, "kernelWeight");
  ...
  for (var i = -1, k = 0; i <= 1; ++i) {
    for (var j = -1; j <= 1; ++j) {
      guasianKernel[k++] = 1.0/(2*Math.PI*sigma*sigma)*Math.exp(-1.0*(i*i + j*j)/(2*sigma*sigma));
    }
  }
  var kernel = guasianKernel;
  gl.uniform1fv(kernelLocation, kernel);
  gl.uniform1f(kernelWeightLocation, computeKernelWeight(kernel));
  console.log(computeKernelWeight(kernel));*/
	//
	// Main render loop
	//
